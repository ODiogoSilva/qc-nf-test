...
IN_genome_size_1_4 = Channel.value(params.genomeSize_1_4)
    .map{it -> it.toString().isNumber() ? it : exit (1, "The genomeSize parameter must be a number or a float. Provided value: '${params.genomeSize_1_4}'")}
IN_min_coverage_1_4 = Channel.value(params.minCoverage_1_4)
    .map{it -> it.toString().isNumber() ? it : exit (1, "The minCoverage parameter must be a number or a float. Provided value: '${params.minCoverage_1_4}'")}

process integrity_coverage2_1_4 {
    ...
    // Send POST request to platform
        ...
        afterScript "final_POST.sh $params.projectId $params.pipelineId 1_4 $params.platformHTTP; report_POST.sh $params.projectId $params.pipelineId 1_4 $params.sampleName $params.reportHTTP $params.currentUserName $params.currentUserId check_coverage_1_4 \"$params.platformSpecies\" true"
    } else {
        beforeScript "PATH=${workflow.projectDir}/bin:\$PATH; set_dotfiles.sh"
        }
    ...
    input:
    set sample_id, file(fastq_pair) from fastqc_out_1_2
    val gsize from IN_genome_size_1_4
    val cov from IN_min_coverage_1_4
    ...
    set sample_id,
        file(fastq_pair),
        file('*_coverage'),
        file('*_max_len') optional true into MAIN_integrity_1_4
    file('*_report') into LOG_report_coverage_1_4
    set sample_id, val("1_4_check_coverage"), file(".status"), file(".warning"), file(".fail"), file(".command.log") into STATUS_check_coverage_1_4
set sample_id, val("check_coverage_1_4"), val("1_4"), file(".report.json"), file(".versions"), file(".command.trace") into REPORT_check_coverage_1_4
file ".versions"
...
check_coverage_out_1_3 = Channel.create()
SIDE_max_len_1_4 = Channel.create()

MAIN_integrity_1_4
    .filter{ it[2].text != "fail" }
    .separate(check_coverage_out_1_3, SIDE_max_len_1_4){
        a -> [ [a[0], a[1]], [a[0], a[3].text]]
    }


process report_coverage2_1_4 {

    // This process can only use a single CPU
    cpus 1
    publishDir 'reports/coverage_1_4/'

    input:
    file(report) from LOG_report_coverage_1_4.filter{ it.text != "corrupt" }.collect()
...
process status {
    ...
    input:
    set sample_id, task_name, status, warning, fail, file(log) from STATUS_integrity_coverage_1_1.mix(STATUS_fastqc_1_2,STATUS_fastqc_report_1_2,STATUS_trimmomatic_1_2,STATUS_fastqc2_1_3,STATUS_fastqc2_report_1_3,STATUS_check_coverage_1_4)
...
process report {
    ...
    set sample_id,
            ...
            report_json,
            version_json,
            trace from REPORT_integrity_coverage_1_1.mix(REPORT_fastqc_1_2,REPORT_fastqc_report_1_2,REPORT_trimmomatic_1_2,REPORT_fastqc2_1_3,REPORT_fastqc2_report_1_3,REPORT_check_coverage_1_4)
